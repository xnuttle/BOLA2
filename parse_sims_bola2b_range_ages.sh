#Xander Nuttle
#parse_sims_bola2b_range_ages.sh
...
#initialize file with number of simulation outcomes matching the observed data for each age's simulation set
echo "age_in_yrs num_sims_matching_data num_sims_with_bola2b_in_only_humans"|sed 's/ /\t/g' > age_results.txt

#compute the number of simulation outcomes matching the observed data for each age's simulation set
for i in $(seq 200000 25000 2000000);
  do echo "/net/gs/vol2/home/xnuttle/bola2_paper/popgen_sims_xn/analysis_programs/filter_scenarios_exact sim${i}_results.txt >> age_results.txt"|bash;
done
