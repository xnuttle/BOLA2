#Xander Nuttle
#sim_bola2b_single_age_no_selection_with_recurrence_650_kya.sh
...
#for j in $(seq -12 1 -3);
#	do i=$(echo "1e$j"); tnano=$(date +%N); tmicro=$(echo "$tnano/1000"|bc -l); t10th_micro=$(echo "$tnano/100"|bc -l); cmd=$(echo "/net/gs/vol2/home/xnuttle/bola2_paper/popgen_sims_xn/analysis_programs/make_command_sim_bola2b_single_age_no_selection_with_recurrence $1 650000 $i"|bash); usleep $tmicro;
...
for i in $(cat /net/eichler/vol18/xander/bola2_popgen_sims/recurrence_rates_to_test.txt);
	do tnano=$(date +%N); tmicro=$(echo "$tnano/1000"|bc -l); t10th_micro=$(echo "$tnano/100"|bc -l); cmd=$(echo "/net/gs/vol2/home/xnuttle/bola2_paper/popgen_sims_xn/analysis_programs/make_command_sim_bola2b_single_age_no_selection_with_recurrence $1 650000 $i"|bash); usleep $tmicro;
    echo "mkdir -p $TMP_DIR;
    chgrp -R eichlerlab $TMP_DIR;
    chmod -R g+wx $TMP_DIR;
    cd $TMP_DIR;
